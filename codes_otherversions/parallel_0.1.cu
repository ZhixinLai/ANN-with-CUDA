...
#define dev_dA2(x, y) dev_A2[x * size_batch + y]     // 2  * 964

#define max_index(x, y) max_index[x * size_batch + y] // 1  * 964
...
int size_Z1 = size_hidden*size_train;
int size_A1 = size_hidden*size_train;
int size_Z2 = size_output*size_train;
int size_A2 = size_output*size_train;

int size_dZ1 = size_hidden*size_train;
int size_dA1 = size_hidden*size_train;
int size_dZ2 = size_output*size_train;
int size_dA2 = size_output*size_train;

int size_max_index = 1*size_train;
...
__global__ void HiddenLayer(double* dev_X, double* dev_W1, double* dev_b1, double* dev_A1, double* dev_Z1, int kind, int size_input, int size_batch)
...
void forward(double* X, int* Y, string type){
  ...
  HiddenLayer<<<size_hidden, size_batch>>>(dev_X, dev_W1, dev_b1, dev_A1, dev_Z1, 1, size_input, size_batch);
  ...
  // output layer and softmax to get Z2
...
__global__ void Back_dA1 (double* dev_W2, double* dev_A1, double* dev_dA1, int size_batch, int size_hidden, int size_output)
...
{  
  ...
  // dA1(20*964) = dZ2(2*964) * W2(2*20)
	...
	for (k = 0; k < size_output; k++)
		partial += dev_W2(k,i) * dev_A1(k,j);
	dev_dA1(i,j) = partial;

}


__global__ void Back_dZ1 (double* dev_dA1, double* dev_A1, double* dev_Z1, double* dev_dZ1, int size_batch, int type)
...
void backprop(int type) {
  ...
  Back_dA1<<<size_hidden, size_train>>> (dev_W2, dev_A1, dev_dA1, size_batch, size_hidden, size_output);    
  ...
  Back_dZ1<<<size_hidden, size_train>>>(dev_dA1, dev_A1, dev_Z1, dev_dZ1, size_train, type);
...
__global__ void update_Wb(double* dev_dWb, double* dev_Wb, int row, int col, double learn_rate)
{
  ...
  for(r = 0; r < row; r++) {
    for(c = 0; c < col; c++) {
      dev_Wb[r*col+c] -= learn_rate * dev_dWb[r*col+c];
    }
  }
...
void updateParameter(double learn_rate)
{
  ...
  cudaMemcpy(dev_W1,   W1,  size_W1  * sizeof(double), cudaMemcpyHostToDevice);
  update_Wb<<<size_hidden, size_input>>>(dev_dW1, W1, size_hidden, size_input, learn_rate);
  ...
  cudaMemcpy(dev_b1,   b1,  size_b1   * sizeof(double), cudaMemcpyHostToDevice);
  update_Wb<<<size_hidden, 1>>>(dev_db1, b1, size_hidden, 1, learn_rate);
  ...
  cudaMemcpy(dev_W2,    W2, size_W2   * sizeof(double), cudaMemcpyHostToDevice);
  update_Wb<<<size_output, size_hidden>>>(dev_dW2, W2, size_output, size_hidden, learn_rate);
  ...
  cudaMemcpy(dev_b2,    b2, size_b2   * sizeof(double), cudaMemcpyHostToDevice);
  update_Wb<<<size_output, 1>>>(dev_db2, b2, size_output, 1, learn_rate);
...
double train(double* X_trn, int* Y_trn) {

  forward(X_trn, Y_trn, "train");
  backprop(1); // 1 ReLU 2 Sigmoid
  updateParameter(0.001);
  return cross_entropy_loss(Y_trn, A2, size_train);
  
}

double test(double* X, int* Y, string type) {
...
int main()
{
  ...
  for(e = 0; e < epochs; e++) {
    read_data();
    initialize_Wb();
    loss = train(X_trn, Y_trn);
    printf("the %d epoch, the training loss is: %f \n", e, loss);
    acc_trn = test(X_trn, Y_trn, "train");
    acc_tst = test(X_tst, Y_tst, "test");
