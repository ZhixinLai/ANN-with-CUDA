...
__global__ void HiddenLayer(double* dev_X, double* dev_W1, double* dev_b1, double* dev_A1, double* dev_Z1, int K, int size_batch, int acti_type, int max_row, int max_col)
...
void forward(double* X, int* Y, string type, int acti_type,  int block_size){
  ...
  for(int i = 0; i < 1000; i++){
    ...
    HiddenLayer<<<dimGrid1, dimBlock>>>(dev_X, dev_W1, dev_b1, dev_A1, dev_Z1, size_input, size_batch, acti_type, size_hidden, size_batch); 
...
void backprop(double* X, int* Y, int acti_type, int block_size) { // type = 1 is Sigmoid
  ...
  Back_dZ1<<<dimGrid5, dimBlock>>>(dev_dA1, dev_A1, dev_Z1, dev_dZ1, size_batch, acti_type, size_hidden, size_batch);
